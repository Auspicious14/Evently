...
export class XIntegrationService {
  ...
  private readonly spamKeywords = [
    ...
    'cashapp', 'venmo', 'zelle', 'bank transfer', 'earn from home', 'passive income', 'mlm', 'pyramid scheme',
    'nft', 'airdrop', 'free crypto', 'pump and dump', 'binary options', 'quick cash', 'side hustle',
    
    'nude', 'porn', 'sex', 'xxx', '18+', 'nsfw', 'hook up', 'one night stand', 'casual encounter',
    'massage service', 'body rub', 'erotic', 'sensual', 'escorts', 'call girl', 'prostitute', 'booty call',
    
    'loan', 'quick loan', 'bad credit', 'no credit check', 'guaranteed approval', 'invest now',
    
    'buy followers', 'increase likes', 'viral', 'hack account', 'recover account', 'password reset',
    'free trial', 'subscription', 'cancel anytime', 'limited time offer', 'act now', 'urgent', 'exclusive deal', 'giveaway scam', 'fake news', 'clickbait'
  ];
  ...
  async handleCron() {
    ...
    try {
      ...
      const baseEventTerms = '(event OR meetup OR conference OR workshop OR summit OR hackathon OR seminar OR webinar)';
      const baseFilters = '-is:retweet -is:reply lang:en';

      const searchQueries = cityGroups.map(group => `${baseEventTerms} (${group}) Nigeria`)
      ...
      for (const batch of queryBatches) {
        const batchPromises = batch.map(query => this.searchWithRetry(query, maxResultsPerQuery));
            ...
            for (const tweet of tweets) {
              try {
                ...
                if (eventData && this.isValidEventData(eventData)) {
                  ...
                  processedCount++;
                  this.logger.log(`Created event from tweet: ${eventData.title} (ID: ${tweet.id})`);
                }
              } catch (error) {
      ...
      this.logger.log(`Processed ${processedCount} Nigerian tweets into events`);
    } catch (error: any) {
      this.logger.error('Failed to fetch or process tweets:', error.message);
    }
  }

  private async searchWithRetry(query: string, maxResults: number, retries = 1): Promise<any[]> {
    try {
      const response = await this.twitterClient.v2.searchAll(query, {
        max_results: maxResults,
        'tweet.fields': ['created_at', 'text', 'entities', 'author_id', 'public_metrics'],
      ...
      if ((error.code === 429 || error.rateLimit) && retries > 0) {
        this.logger.warn(`Rate limit hit for query "${query}", retrying after 60s...`);
  ...
  async postApprovedEvents() {
    ...
    try {
      ...
      for (let i = 0; i < events.length; i += batchSize) {
        ...
        const batchPromises = batch.map(event => this.postEventWithRetry(event));
        const results = await Promise.allSettled(batchPromises);
  ...
  private async postEventWithRetry(event: any, retries = 1): Promise<void> {
    try {
      ...
      if ((error.code === 429 || error.rateLimit) && retries > 0) {
        this.logger.warn(`Rate limit hit for posting event ${event._id}, retrying after 60s...`);
        await this.delay(60000);
  ...
  private isSpamOrInappropriate(text: string): boolean {
    ...
    if (linkCount > 2 || text === text.toUpperCase() || (text.match(/[!?.]{3,}/g) || []).length > 2) {
      return true;
    }
  ...
  private isNigeriaRelated(text: string): boolean {
    ...
    return this.nigeriaLocations.some(location => lowerText.includes(location));
  }
  ...
  private formatEventTweet(event: any): string {
    ...
    const remainingSpace = maxLength - tweet.length - eventUrl.length - hashtags.length - 10;
  ...
  private isLikelyEvent(text: string): boolean {
    ...
    const strongKeywords = [
      'conference', 'summit', 'workshop', 'seminar', 'hackathon', 'meetup', 'tech event',
      'startup event', 'developer conference', 'join us', 'register now', 'save the date',
      'rsvp', 'tickets available', 'upcoming event', 'webinar', 'panel discussion',
      'networking event', 'launch event', 'demo day', 'pitch night', 'google'
    ];

    const hasStrongKeyword = strongKeywords.some(keyword => lowerText.includes(keyword));
    
    const hasDateIndicator = /\d{1,2}[\/\-]\d{1,2}([\/\-]\d{2,4})?|\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|june|july|august|september|october|november|december)\b/i.test(text) ||
                             /\b(next|this|upcoming)\s+(week|month|friday|saturday|sunday|monday|tuesday|wednesday|thursday)\b/i.test(lowerText);
  ...
  private extractTitle(text: string): string {
    ...
    const eventKeywords = ['event', 'meetup', 'conference', 'workshop', 'summit', 'hackathon'];
    for (const line of lines) {
      if (line.length > 15 && line.length < 120 && /^[A-Z]/.test(line) && !this.isBadTitle(line) &&
  ...
  private cleanDescription(text: string): string {
    const emojiRegex = /[\u{1F000}-\u{1FFFF}]/gu;
    let cleaned = text.replace(/https?:\/\/\S+/g, '').replace(/@\w+/g, '').replace(emojiRegex, '');
  ...
  private extractDate(text: string, fallbackDate: string): Date | null {
    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        ...
        if (!isNaN(parsedDate.getTime()) && parsedDate > new Date(Date.now() - 24 * 60 * 60 * 1000)) {  // Allow recent but focus on future
          return parsedDate;
        }
      }
    }

    const fallback = new Date(new Date(fallbackDate).getTime() + 7 * 24 * 60 * 60 * 1000);
    return fallback;
  }
  ...
  private extractLocation(text: string): string | null {
    ...
    for (const location of this.nigeriaLocations.sort((a, b) => b.length - a.length)) {
  ...
  private determineCategory(text: string): string {
    const categoryKeywords = {
      'AI': ['ai', 'artificial intelligence', 'machine learning', 'ml', 'deep learning', 'chatgpt', 'llm', 'neural network'],
      'Fintech': ['fintech', 'financial', 'banking', 'payment', 'blockchain', 'crypto', 'defi', 'web3'],
      'Startup': ['startup', 'entrepreneur', 'founder', 'business', 'pitch', 'vc', 'funding', 'accelerator', 'incubator'],
      'Coding': ['coding', 'programming', 'developer', 'software', 'hackathon', 'dev', 'engineer', 'code', 'api'],
      'Hardware': ['hardware', 'iot', 'robotics', 'electronics', 'embedded', 'arduino', 'raspberry pi'],
      'Design': ['design', 'ui', 'ux', 'creative', 'figma', 'product design', 'graphic design'],
      'Marketing': ['marketing', 'growth', 'sales', 'branding', 'seo', 'content', 'digital marketing', 'social media'],
      'Cybersecurity': ['cybersecurity', 'security', 'infosec', 'hacking', 'privacy', 'pen test', 'ethical hacking'],
      'Virtual': ['virtual', 'online', 'remote', 'webinar', 'zoom', 'virtual event', 'live stream'],
      'HealthTech': ['healthtech', 'medtech', 'healthcare', 'telemedicine', 'ehealth'],
  ...
  private checkIfFree(text: string): boolean {
    const freeKeywords = [
      'free', 'no cost', 'complimentary', 'free admission', 'free entry', 'free event',
      'free registration', 'zero cost', 'open to all', 'no ticket required', 'gratis'
    ];
    const paidKeywords = ['ticket', 'buy', 'purchase', 'fee', 'paid', 'register and pay', 'entrance fee'];
    const lowerText = text.toLowerCase();

    if (paidKeywords.some(keyword => lowerText.includes(keyword))) return false;
  ...
  private isValidEventData(eventData: CreateEventDto): boolean {
    ...
    if (!eventData.description || eventData.description.length < 50) return false;
    if (!eventData.location) return false;
    if (new Date(eventData.date) < new Date(Date.now() - 24 * 60 * 60 * 1000)) return false;
    return true;
  }
